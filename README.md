...
It ships with a catalogue of **204 spaces**, one simplicial model per
topological type, drawn from SageMath's example catalogue, Frank Lutz's Manifold
Page and the literature. For the 198 that are computable it publishes **2772
JSON records** — a cup product ring and a homology group for every space over
each of `ZZ`, `QQ`, `GF(2)`, `GF(3)`, `GF(5)`, `GF(7)` and `GF(11)`. **No triangulations
...
* **SageMath is optional.** 33 of the 204 catalogue entries are built by
...
The catalogue holds **204 entries**, one simplicial model per topological type.
...
|---|---:|---|---|
| catalogue, computable | **198** | all seven rings | all seven rings |
| catalogue, marked `heavy` | **6** | none | none |
| **total** | **204** | | |

For the 198 computable entries the corpus is uniform: every space has a
cohomology ring **and** homology over `ZZ`, `QQ`, `GF(2)`, `GF(3)`, `GF(5)`,
`GF(7)` and `GF(11)` — 1386 records of each kind. There are no partial spaces
and no ring left to a single coefficient ring.

Three qualifications on what a ring record contains:

* **Multiplication tables: 1355 of 1386 are complete.** The 31 exceptions are
...
* **Torsion-free spaces carry redundancy.** Where `H^*(K;ZZ)` is torsion-free
  the other six rings follow from it by universal coefficients, so those
...
* **`GF(11)` is a control ring, and one space justifies it.** Scanning every
  integral invariant factor in the corpus, the torsion primes present are 2, 3,
  5 and 7 — so over `GF(11)` every other space must agree with `QQ` exactly, and
  measurably does. That makes it a standing consistency check: a deviation is a
  bug, not a discovery. The exception is `M(Z/11,2)`, added with the ring
  precisely so it is not vacuous; it is the only space in the catalogue whose
  `GF(11)` cohomology differs from its rational cohomology.
...
| prime order | `GF(2)`, `GF(3)`, `GF(5)`, `GF(7)`, `GF(11)` |
| prime power order | `GF(4) = F_2^2`, `GF(8) = F_2^3`, `GF(9) = F_3^2`, `GF(25) = F_5^2` |

**Result over the whole corpus: 198 spaces × 11 rings = 2178 comparisons, zero
disagreements.**
...
| check | scope | result |
|---|---|---|
| OSCAR/Polymake integral homology | every space | 198 / 198 agree |
| Independent integral homology, re-derived here (`--deep`) | 196 of 198 (two too large) | 196 / 196 agree |
...
julia scripts/run_tables.jl                            # all 198 computable spaces
...
cover all seven rings for every computable space. Over a field coordinates are
...
the same fixed grid — `ZZ`, `QQ`, `GF(2)`, `GF(3)`, `GF(5)`, `GF(7)`, `GF(11)`.
Homology
...
the vertex order). As shipped that is 1386 of each, 2772 records over 198
spaces, about 47 MB.
...
records of 228; the affected families are the same across the seven-ring grid.
...
* **the table** — row class ∪ column class, in the basis of the target degree. A
  ...
  for `d > dim K`.
* **`ring structure`** — generators and relations, e.g.
  `Q[x3]/(x3^2),  |x3| = 3` for a single generator, `Lambda_Z(x1_1, x1_2)` for
  the torus, or `Z[x2,x3]/(3*x2, x2^2, x2*x3, x3^2)` for `L(3,1)`. Generators
  are classes of the canonical basis, so their names are the same `x2_1` labels
  the table uses. The presentation separates spaces the groups alone do not:
  `S^2xS^2` is `Z[x2_1,x2_2]/(x2_1^2, x2_2^2)` while `CP^2#CP^2` is
  `Z[x2_1,x2_2]/(x2_1*x2_2, x2_2^2 - x2_1^2)`. Above the caps in Limitations it
  says to read the table instead.
