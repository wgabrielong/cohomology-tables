...
Cup product ring records do name the individual faces in the support of each
cocycle representative — the faces a chosen cochain is non-zero on. That is a
small subset of the faces, only in dimensions carrying cohomology, and it does
not determine the complex. If you consider even that too close to the upstream
data for your purposes, drop the `cocycle_representatives` field when
regenerating.

### Why homology records carry no cycle representatives

Homology records name **no faces at all**, and there is no flag to make them.
The asymmetry with cup product records is deliberate and it is forced.

For a closed orientable `n`-manifold, `H_n` is generated by the fundamental
class, which is the signed sum of *every* `n`-simplex. In top degree there are
no boundaries to quotient by, so that representative is unique — there is no
sparser choice. A top-degree cycle representative therefore **is** the canonical
facet list, with an orientation sign attached to each facet. Measured on the
catalogue:

| space | facets | support of the top-degree generator |
|---|---|---|
| `S^2` | 4 | 4 |
| `T^2` | 14 | 14 |
| `RP^3` | 40 | 40 |
...
Emitting those would redistribute exactly the input this repository does not
ship, and no support threshold rescues it: the property holds for every
orientable closed manifold in the catalogue, which is most of the interesting
ones. Cohomology is safe here by the dual accident — a cocycle can be, and in
positive degrees measurably is, supported on a single face.

`cycle_representative(X, d, j)` computes one locally, from a recipe that is
already reproducible. It is simply never serialised.
